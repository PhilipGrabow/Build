...
public class OnlineTime implements Listener {
	...
	public void onJoin(final PlayerJoinEvent e) {
		...
		if(cfg.getInt("OnlineTime." + e.getPlayer().getName()) >=60) {
			...
			e.getPlayer().sendMessage("§bDeine OnlineZeit beträgt zurzeit : " + cfg.getInt("OnlineTime." + e.getPlayer().getName()) + " Minute/n !");
		}
		Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
			...
			public void run() {
				...
				if(cfg.getInt("OnlineTime." + e.getPlayer().getName()) == belohnung1time) {
					...
					e.getPlayer().sendMessage("Nächste Belohnung bei " + belohnung2time + " Minuten !");
